...
class ActinAnalyzer:
    def __init__(self, h5_file_path, box_size, analyze_reactions=True):
        ...
        self.times, self.types, self.ids, self.positions = self.traj.read_observable_particles()
    ...
    def _filamentous_actin_types():
        ...
        """
        return (ActinAnalyzer._pointed_actin_types() + ActinAnalyzer._middle_actin_types() 
                + ActinAnalyzer._barbed_actin_types() + ActinAnalyzer._branch_actin_types())
    ...
    def _get_frame_filaments_from_start_actins(start_actin_ids, frame_particle_data):
        ...
        """
        result = []
        non_start_actin_types = ActinAnalyzer._middle_actin_types() + ActinAnalyzer._barbed_actin_types()
        for start_actin_id in start_actin_ids:
            result.append(ReaddyUtil.analyze_frame_get_chain_of_types(
                start_actin_id, non_start_actin_types, frame_particle_data, result=[start_actin_id]))
        return result
    ...
    def _frame_mother_filaments(frame_particle_data):
        ...
        return ActinAnalyzer._get_frame_filaments_from_start_actins(
            ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._pointed_actin_types(), frame_particle_data), 
            frame_particle_data
        )
    ...
    def _frame_daughter_filaments(frame_particle_data):
        ...
        """
        return ActinAnalyzer._get_frame_filaments_from_start_actins(
            ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._branch_actin_types(), frame_particle_data), 
            frame_particle_data
        )
    ...
    def analyze_ratio_of_filamentous_to_total_actin(self):
        ...
        for t in range(len(self.particle_data)):
            free_actin = len(ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._free_actin_types(), self.particle_data[t]))
            filamentous_actin = len(ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._filamentous_actin_types(), self.particle_data[t]))
    ...
    def analyze_ratio_of_ATP_actin_to_total_actin(self):
        ...
        result = []
        for t in range(len(self.particle_data)):
            ATP_actin = len(ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._ATP_actin_types(), self.particle_data[t]))
            ADP_actin = len(ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._ADP_actin_types(), self.particle_data[t]))
    ...
    def analyze_ratio_of_daughter_filament_actin_to_total_filamentous_actin(self):
        """
        Get a list of the ratio [daughter filament actin] / [filamentous actin] over time
        """
        result = []
        for t in range(len(self.particle_data)):
            mother_actin = 0
            mother_filaments = ActinAnalyzer._frame_mother_filaments(self.particle_data[t])
            for mother_filament in mother_filaments[t]:
                mother_actin += len(mother_filament)
            daughter_actin = 0
            daughter_filaments = ActinAnalyzer._frame_daughter_filaments(self.particle_data[t])
            for daughter_filament in [t]:
    ...
    def analyze_ratio_of_bound_to_total_arp23(self):
        ...
        for t in range(len(self.particle_data)):
            bound_arp23 = 0
            free_arp23 = 0
            arp3_ids = ReaddyUtil.analyze_frame_get_ids_for_types(["arp3", "arp3#branched"], self.particle_data[t])
    ...
    def analyze_ratio_of_capped_ends_to_total_ends(self):
        ...
        growing_end_types = ActinAnalyzer._barbed_actin_types() + ["actin#branch_barbed_1", "actin#branch_barbed_ATP_1"]
        result = []
        for t in range(len(self.particle_data)):
            capped_ends = len(ReaddyUtil.analyze_frame_get_ids_for_types(capped_end_types, self.particle_data[t]))
            growing_ends = len(ReaddyUtil.analyze_frame_get_ids_for_types(growing_end_types, self.particle_data[t]))
    ...
    def _get_frame_branch_ids(frame_particle_data):
        ...
        """
        arp2_ids = ReaddyUtil.analyze_frame_get_ids_for_types(["arp2#branched"], frame_particle_data)
        actin_types = ActinAnalyzer._pointed_actin_types() + ActinAnalyzer._middle_actin_types() + ActinAnalyzer._barbed_actin_types()
        result = []
        for arp2_id in arp2_ids:
            actin1_id = ReaddyUtil.analyze_frame_get_id_for_neighbor_of_types(arp2_id, ActinAnalyzer._branch_actin_types(), frame_particle_data)
            if actin1_id is None:
                raise Exception("couldn't parse branch point: failed to find branch actin")
            ...
            if len(branch_actins) < 4:
                print(f"couldn't parse branch point: only found {len(branch_actins)} daughter actins")
                continue
            actin_arp2_id = ReaddyUtil.analyze_frame_get_id_for_neighbor_of_types(arp2_id, actin_types, frame_particle_data, [actin1_id])
            if actin_arp2_id is None:
                raise Exception("couldn't parse branch point: failed to find actin_arp2")
            n = ReaddyUtil.calculate_polymer_number(int(frame_particle_data[actin_arp2_id][0][-1:]), -1)
            actin_arp3_types = ["actin#{}".format(n), "actin#ATP_{}".format(n),
                                "actin#pointed_{}".format(n), "actin#pointed_ATP_{}".format(n),
                                "actin#branch_{}".format(n), "actin#branch_ATP_{}".format(n)]
            actin_arp3_id = ReaddyUtil.analyze_frame_get_id_for_neighbor_of_types(actin_arp2_id, actin_arp3_types, frame_particle_data)
            if actin_arp3_id is None:
                raise Exception("couldn't parse branch point: failed to find actin_arp3")
            main_actins = ReaddyUtil.analyze_frame_get_chain_of_types(
                actin_arp3_id, actin_types, frame_particle_data, 2, actin_arp2_id, [actin_arp2_id, actin_arp3_id])
            if len(main_actins) < 4:
                print(f"couldn't parse branch point: only found {len(main_actins)} mother actins")
    ...
    def _get_frame_branch_angles(frame_particle_data):
        ...
        for branch in branch_ids:
            main_pos1 = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, [branch[0], branch[1], branch[2]])
            if main_pos1 is None or ReaddyUtil.vector_is_invalid(main_pos1):
                raise Exception("Failed to axis position for mother actin 1")
            main_pos2 = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, [branch[1], branch[2], branch[3]])
            ...
            branch_pos1 = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, [branch[4], branch[5], branch[6]])
            ...
            branch_pos2 = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, [branch[5], branch[6], branch[7]])
    ...
    def analyze_branch_angles(self):
        ...
        result = []
        for t in range(len(self.particle_data)):
            branch_angles = ActinAnalyzer._get_frame_branch_angles(self.particle_data[t])
    ...
    def _calculate_pitch(frame_particle_data, actin1_ids, actin2_ids, box_size):
        ...
        actin1_axis_pos = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, actin1_ids)
        ...
        actin2_axis_pos = ActinAnalyzer._get_axis_position_for_actin(frame_particle_data, actin2_ids)
    ...
    def _get_frame_short_helix_pitches(frame_particle_data, box_size):
        ...
        for filament in filaments:
            for i in range(1, len(filament)-3):
                short_pitch = _calculate_pitch(
                    frame_particle_data, [filament[i-1], filament[i], filament[i+1]], [filament[i], filament[i+1], filament[i+2]], box_size)
    ...
    def _get_frame_long_helix_pitches(frame_particle_data, box_size):
        ...
        for filament in filaments:
            for i in range(1, len(filament)-3):
                long_pitch = _calculate_pitch(
                    frame_particle_data, [filament[i-1], filament[i], filament[i+1]], [filament[i+1], filament[i+2], filament[i+3]], box_size)
    ...
    def analyze_short_helix_pitches(self):
        ...
        result = []
        for t in range(len(self.particle_data)):
            helix_pitches = ActinAnalyzer._get_frame_short_helix_pitches(self.particle_data[t], self.box_size)
    ...
    def analyze_long_helix_pitches(self):
        ...
        result = []
        for t in range(len(self.particle_data)):
            helix_pitches = ActinAnalyzer._get_frame_long_helix_pitches(self.particle_data[t], self.box_size)
            result.append(helix_pitches)
        return result
        ...
        for filament in filaments:
            ...
            for i in range(1,len(filament)-1):
                ...
                if axis_pos is None or ReaddyUtil.vector_is_invalid(axis_pos):
                    raise Exception(f"Failed to axis position for actin[{i}] on filament {filament}")
                if np.linalg.norm(axis_pos - last_pos) < box_size / 2.: # make sure we didn't cross periodic boundary
                    print("filament crossed periodic boundary, skipping straightness calculation")
                    continue
                positions.append(axis_pos)
                last_pos = axis_pos
            if len(positions) > 2:
                axis = ActinAnalyzer._calculate_line(np.squeeze(np.array(positions)), box_size)
    ...
    def analyze_filament_straightness(self):
        """
        Get a list of the distances from each actin axis position to the ideal axis position
        on each filament in each frame of the trajectory
        """
        result = []
        for t in range(len(self.particle_data)):
            straightness = ActinAnalyzer._get_frame_distance_from_straight(self.particle_data[t], self.box_size)
    ...
    def _get_reaction_type(readdy_reaction_name, reactions):
        ...
        raise Exception("couldn't find reaction named {}".format(readdy_reaction_name))

    @staticmethod
    def _get_readdy_reaction_events_over_time(readdy_reaction_name, multiplier, stride, result, reactions):
        """
        Get a list of the number of times a ReaDDy reaction has happened by each time step
        """
    ...
    def analyze_reaction_events_over_time(self, total_reaction_name):
        ...
        for reaction_name in readdy_reactions[0]:
            result = ActinAnalyzer._get_readdy_reaction_events_over_time(reaction_name, 1, 1, result, self.reactions)
    ...
    def analyze_all_reaction_events_over_time(self):
        ...
        for total_reaction_name in total_reactions:
            result[total_reaction_name] = self.analyze_reaction_events_over_time(total_reaction_name)
            sys.stdout.write('\r')
            p = 100. * (i + 1) / float(len(total_reactions))
            sys.stdout.write("Analyzing reactions [{}{}] {}%".format('='*int(round(p)), ' '*int(100. - round(p)), round(10. * p) / 10.))
            sys.stdout.flush()
            i += 1
        return result
    ...
    def analyze_free_actin_concentration_over_time(self):
        ...
        result = []
        for t in range(len(self.particle_data)):
            result.append(ReaddyUtil.calculate_concentration(
                len(ReaddyUtil.analyze_frame_get_ids_for_types(ActinAnalyzer._free_actin_types(), self.particle_data[t])), self.box_size))
        return result
