...
class KinesinUtil:
    ...
    def set_kinesin_state(topology, recipe, from_motor_state, to_motor_state):
        ...
        if len(motors) < 2:
            raise Exception(f"Failed to find 2 motors, found {len(motors)}\n"
        ...
        if "ADP" in to_motor_state and "ADP" in other_state:
            ...
            new_states.sort()
            recipe.change_topology_type(f"Microtubule-Kinesin#{new_states[0]}-{new_states[1]}")
        return motor_to_set
    ...
    def reaction_function_motor_bind_tubulin(topology):
        ...
        if motor is None:
            raise Exception("Failed to find new motor\n"
                            + ReaddyUtil.topology_to_string(topology))
        tubulin = ReaddyUtil.get_neighbor_of_type(
            topology, motor, "tubulinB#bound", False)
        if tubulin is None:
            raise Exception("Failed to find bound tubulin\n"
                            + ReaddyUtil.topology_to_string(topology))
        if parameters["verbose"]:
            print("{} ++ {}".format(
    ...
    def reaction_function_motor_bind_ATP(topology):
        ...
        if motor is None:
            raise Exception("Failed to find motor in apo state\n"
                            + ReaddyUtil.topology_to_string(topology))
        if parameters["verbose"]:
    ...
    def reaction_function_motor_release_tubulin(topology):
        ...
        if motor is None:
            raise Exception("Failed to find motor in ATP state\n"
                            + ReaddyUtil.topology_to_string(topology))
        tubulin = ReaddyUtil.get_neighbor_of_type(
            topology, motor, "tubulinB#bound", False)
        if tubulin is None:
            raise Exception("Failed to find bound tubulin\n"
                            + ReaddyUtil.topology_to_string(topology))
        if parameters["verbose"]:
            print(ReaddyUtil.vertex_to_string(topology, motor) + " -X- " + 
                  ReaddyUtil.vertex_to_string(topology, tubulin))
    ...
    def add_tubulin_bonds_and_repulsions(tubulin_types, force_constant, system, util):
        ...
        for t in range(len(tubulin_types)):
            all_tubulin_types += MicrotubulesUtil.get_all_polymer_tubulin_types(tubulin_types[t])
