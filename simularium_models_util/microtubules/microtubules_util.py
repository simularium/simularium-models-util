...
class MicrotubulesUtil:
    ...
    def get_neighboring_tubulin(topology, vertex, polymer_offsets):
        ...
        return ReaddyUtil.get_neighbor_of_type(
            topology, vertex,
            MicrotubulesUtil.polymer_indices_to_string(MicrotubulesUtil.increment_polymer_indices(
    ...
    def get_polymer_offsets(particle_types):
        ...
        """
        polymer_indices = [MicrotubulesUtil.get_polymer_indices(particle_types[0]),
    ...
    def get_filament_lengths(topology, tubulin_minus, tubulin_plus):
        ...
        """
        next_tub_minus = MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin_minus, [-1, 0])
        next_tub_plus = MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin_plus, [1, 0])
    ...
    def filament_is_crosslinked(topology, tubulin, direction):
        ...
        for i in range(2):
            ...
            if neighbor_tub is not None:
                return True
        next_tub = MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin, [direction, 0])
        if next_tub is None:
            return False
        return MicrotubulesUtil.filament_is_crosslinked(topology, next_tub, direction)

    @staticmethod
    def remove_bent_filament_site_bonds(
        topology, recipe, tubulin_minus, tubulin_plus
    ):
        """
        ...
        if v_sites_minus is None:
            raise Exception("Failed to find sites on bent tubulin (" + 
    ...
    def topology_is_microtubule(topology):
        ...
        for vertex in topology.graph.get_vertices():
            ...
            if "tubulin" in pt:
                if MicrotubulesUtil.get_neighboring_tubulin(topology, vertex, [0, -1]) is not None:
                    return True
                if MicrotubulesUtil.get_neighboring_tubulin(topology, vertex, [0, 1]) is not None:
                    return True
        return False
    ...
    def tubulin_is_crosslinked(topology, tubulin):
        ...
        """
        return (MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin, [0, -1]) is not None and
                MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin, [0, 1]) is not None)

    @staticmethod
    def check_remove_tubulin_sites(
        topology, recipe, tubulin, tubulin_crosslinked=False, 
        neighbor_tubulin=None, neighbor_crosslinked=False
    ):
        """
        ...
        if not MicrotubulesUtil.tubulin_has_sites(topology, tubulin):
            return False
        crosslinked = []
        neighbor_tubulins = [MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin, [-1, 0]),
        ...
        for nt in range(2):
            if (neighbor_tubulin is not None and neighbor_tubulins[nt] is not None
                ...
                if neighbor_tubulins[nt] is not None:
                    crosslinked.append(
                        MicrotubulesUtil.tubulin_is_crosslinked(topology, neighbor_tubulins[nt]))
                else:
                    crosslinked.append(nt == 0)
        if (False not in crosslinked and
            (tubulin_crosslinked or MicrotubulesUtil.tubulin_is_crosslinked(topology, tubulin))):
    ...
    def tubulin_is_not_crosslinked(topology, tubulin):
        ...
        """
        return (MicrotubulesUtil.get_neighboring_tubulin(topology, tubulin, [0, -1]) is None or
    ...
    def check_add_tubulin_sites(
        ...
        if MicrotubulesUtil.tubulin_has_sites(topology, tubulin):
            ...
            return False
        MicrotubulesUtil.add_tubulin_sites(topology, recipe, tubulin, site1_type, site2_type)
        return True
    ...
    @staticmethod
    def setup_sites(
        topology, recipe, v_new_sites, position, side, normal, 
        tangent, site_state_ring, site_state_filament
    ):
        """
    ...
    @staticmethod
    def connect_sites_between_tubulins(
        topology, recipe, v_sites_minus, v_sites_plus
    ):
        """
    ...
    @staticmethod
    def add_edges(
        microtubule, n_filaments, n_rings, 
    ...
    def add_tubulin_types(system, diffCoeff):
        ...
        for tubulin_type in all_tubulin_types:
            MicrotubulesUtil.add_polymer_topology_species(tubulin_type, diffCoeff, system)
    ...
    def do_grow1(topology, GTP_state):
        ...
        if v_newB is None:
            raise Exception("Failed to find tubulinB#free vertex\n"
                            + ReaddyUtil.topology_to_string(topology))
        v_newA = ReaddyUtil.get_neighbor_of_type(
            topology, v_newB, "tubulinA#free", True)
        if v_newA is None:
            raise Exception("Failed to find tubulinA#free vertex\n"
                            + ReaddyUtil.topology_to_string(topology))
    ...
    def do_grow2(topology):
        ...
        MicrotubulesUtil.connect_sites_between_tubulins(topology, recipe, v_sites, v_new_sitesA)
        # new tubulinB
        tangent = ReaddyUtil.rotate(np.copy(tangent), side, np.deg2rad(10.))
        normal = ReaddyUtil.rotate(np.copy(normal), side, np.deg2rad(10.))
        ...
        MicrotubulesUtil.connect_sites_between_tubulins(topology, recipe, v_new_sitesA, v_new_sitesB)
        recipe.change_topology_type("Microtubule")
        return recipe
    ...
    def do_shrink1(topology, GTP_state):
        ...
        if (MicrotubulesUtil.filament_is_crosslinked(topology, tubulins[0], -1) and
            ...
            if parameters["verbose"]:
                print("Shrink cancelled: both fragments "
                    ...
                    f"and {ReaddyUtil.vertex_to_string(topology, tubulins[1])}) are crosslinked")
            return recipe
        # will either of the cut fragments be a dimer?
        is_dimer = []
        filament_lengths = MicrotubulesUtil.get_filament_lengths(topology, tubulins[0], tubulins[1])
        ...
        if not is_dimer[0]:
            ...
            if site4 is None:
                raise Exception("Shrink cancelled: Couldn't find site4 to set reactive\n" 
                                + ReaddyUtil.topology_to_string(topology))
        ...
        # detach at least one dimer
            ...
            for i in range(len(tubulins)):
                ...
                tubulins_to_detach.append(MicrotubulesUtil.get_neighboring_tubulin(
                    topology, tubulins[i], [-1 if i == 0 else 1, 0]))
            if not MicrotubulesUtil.set_free(topology, recipe, tubulins_to_detach):
                raise Exception("Failed to find sites for tubulins being released\n"
                                + ReaddyUtil.topology_to_string(topology))
        removed, message = ReaddyUtil.try_remove_edge(
            topology, recipe, tubulins[0], tubulins[1])
        if not removed:
    ...
    def reaction_function_attach(topology):
        ...
        if len(attaching_sites) == 0:
            raise Exception("Failed to find attaching vertices\n"
                            + ReaddyUtil.topology_to_string(topology))
        tubulins = []
        for site in attaching_sites:
            ...
            if tubulins[len(tubulins)-1] is None:
                raise Exception("Attach cancelled: failed to find attaching site's "
        ...
        if not MicrotubulesUtil.tubulins_can_attach(tubulin_types):
            MicrotubulesUtil.cancel_attach(topology, recipe, attaching_sites)
            if parameters["verbose"]:
                print(f"Attach cancelled: tubulins (" +
                      ReaddyUtil.vertex_to_string(topology, tubulins[0]) + ", " + 
                      ReaddyUtil.vertex_to_string(topology, tubulins[1]) + ") can't attach")
            return recipe
        for i in range(2):
            crosslinked = MicrotubulesUtil.get_neighboring_tubulin(
                topology, tubulins[i], [0, 1 if i == 0 else -1]) is not None
            if crosslinked:
                MicrotubulesUtil.check_remove_tubulin_sites(topology, recipe, tubulins[i], True)
            else:
                ReaddyUtil.set_flags(topology, recipe, tubulins[i], [], ["bent"])
            prev_tubulin = MicrotubulesUtil.get_neighboring_tubulin(topology, tubulins[i], [-1, 0])
            if prev_tubulin is not None:
                MicrotubulesUtil.check_remove_tubulin_sites(topology, recipe, prev_tubulin, False,
                    tubulins[i], crosslinked)
            next_tubulin = MicrotubulesUtil.get_neighboring_tubulin(topology, tubulins[i], [1, 0])
            if next_tubulin is not None:
                MicrotubulesUtil.check_remove_tubulin_sites(topology, recipe, next_tubulin, False,
                    tubulins[i], crosslinked)
        removed, message = ReaddyUtil.try_remove_edge(
    ...
    def reaction_function_detach1(topology):
        ...
        if parameters["verbose"]:
            print("Detach")
        recipe = readdy.StructuralReactionRecipe(topology)
        GTP_state = ("GTP" if (random.random() <= parameters["ring_detach_GTP_rate"] /
            (parameters["ring_detach_GTP_rate"] + parameters["ring_detach_GDP_rate"])) else "GDP")
    ...
    def reaction_function_detach2(topology):
        ...
        if None in detaching_sites:
            ...
            raise Exception(f"Failed to find detaching sites, ({site0}, {site1})\n" 
                            + ReaddyUtil.topology_to_string(topology))
        detaching_tubulins = [
            ReaddyUtil.get_neighbor_of_type(
        ...
        if None in detaching_tubulins:
            ...
            raise Exception(f"Failed to find detaching tubulins ({tub0}, {tub1})\n" 
                            + ReaddyUtil.topology_to_string(topology))
        for i in range(2):
            tubulins = [MicrotubulesUtil.get_neighboring_tubulin(
            ...
            if tubulins[0] is None and tubulins[2] is None:
                raise Exception("Tubulin has no neighbors on filament?!\n" 
                                + ReaddyUtil.topology_to_string(topology))
            ...
            for j in range(3):
                ...
                if tubulins[j] is not None:
                    ...
                    if sites[j] is None:
                        ...
                        if sites[j] is None:
                            raise Exception(ReaddyUtil.vertex_to_string(topology, tubulins[j])
                                            + f" is missing sites [{i}]\n" 
                                            + ReaddyUtil.topology_to_string(topology))
                        added_sites[j] = True
                        site_state_filament = (tubulin_state
                            if MicrotubulesUtil.get_neighboring_tubulin(topology, tubulins[j], [1, 0])
                            is None else "")
                        MicrotubulesUtil.setup_sites(topology, recipe, sites[j],
